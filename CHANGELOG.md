...
- **`AGENTMEMORY_IDLE_CHECKPOINT_MS` / `AGENTMEMORY_IDLE_CHECKPOINT_POLL_MS` / `AGENTMEMORY_IDLE_CHECKPOINT_ENABLED` env vars + idle-checkpoint poll** - a server-side background poll ([`src/index.ts`](src/index.ts), default every 180 000 ms, gated by `AGENTMEMORY_IDLE_CHECKPOINT_ENABLED`, started only when the threshold is positive) consolidates each active session that has been idle past `AGENTMEMORY_IDLE_CHECKPOINT_MS` (default 600 000 ms) via a new `mode: "idle-checkpoint"` on `mem::session-sweep` ([`src/functions/session-sweep.ts`](src/functions/session-sweep.ts)), keeping the session active. `getIdleCheckpointMs()` lives in [`src/config.ts`](src/config.ts). `POST /agentmemory/session/sweep` accepts and validates the new `mode` field ([`src/triggers/api.ts`](src/triggers/api.ts)).
