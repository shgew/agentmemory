...
- **OpenCode commit-link posted repo URLs with embedded credentials.** The `repo` field came straight from `git remote get-url origin`, which can carry userinfo (`https://user:TOKEN@host/…`). A new module-private `sanitizeRepoUrl()` strips any userinfo before the metadata leaves the machine (scp-style `git@host:path` remotes have no userinfo and pass through unchanged).
...
- **Pending observation recovery ignored the compression opt-out and scanned all raw history.** Recovery now rebuilds synthetic observations when `AGENTMEMORY_AUTO_COMPRESS` is disabled. A per-session pending index gives checkpoints and summaries targeted raw reads, while session sweep keeps one global pass to discover legacy rows.
...
- **OpenCode commit provenance attributed unrelated HEAD changes and dropped intermediate commits.** Each shell tool now gets a unique reflog marker. Checkout, reset, pull fast-forward, concurrent-session movement, and rebase bookkeeping are ignored; created commits post oldest first and failed suffixes retry without duplicating successful links.
