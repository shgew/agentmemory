...
- **`AGENTMEMORY_GRAPH_EXTRACT_TIMEOUT_MS` env var** - configurable wall-clock cap for the `mem::graph-extract` orchestration (default 180 000 ms, matching the previously-hardcoded ceiling). Passed as `sdk.trigger`'s `timeoutMs` at the `event::session::stopped` consolidation site ([`src/triggers/events.ts`](src/triggers/events.ts)), the `POST /agentmemory/graph/extract` REST handler ([`src/triggers/api.ts`](src/triggers/api.ts)), and the per-batch `POST /agentmemory/graph/build` backfill loop. Distinct from `AGENTMEMORY_LLM_TIMEOUT_MS`, which bounds individual outbound LLM/embedding fetches. Resolves the recurring `Invocation timeout after 180000ms: mem::graph-extract` failure on large sessions (hundreds of compressed observations passed to a single graph-extract call) when extraction runs on slow local models.
- **`AGENTMEMORY_IDLE_CHECKPOINT_MS` / `AGENTMEMORY_IDLE_CHECKPOINT_POLL_MS` / `AGENTMEMORY_IDLE_CHECKPOINT_ENABLED` env vars + idle-checkpoint poll** - a server-side background poll ([`src/index.ts`](src/index.ts), default every 180 000 ms, gated by `AGENTMEMORY_IDLE_CHECKPOINT_ENABLED`, started only when the threshold is positive and graph extraction is enabled) extracts the new graph window for each active session idle past `AGENTMEMORY_IDLE_CHECKPOINT_MS` (default 600 000 ms) via `mode: "idle-checkpoint"` on `mem::session-sweep` ([`src/functions/session-sweep.ts`](src/functions/session-sweep.ts)), keeping the session active. `getIdleCheckpointMs()` lives in [`src/config.ts`](src/config.ts). `POST /agentmemory/session/sweep` accepts and validates the `mode` field ([`src/triggers/api.ts`](src/triggers/api.ts)).
- **`OPENAI_EMBEDDING_MAX_BATCH` env var** - max inputs per `/embeddings` request before splitting into sequential sub-batches (default 256). Some self-hosted OpenAI-compatible runners (e.g. Ollama) crash the model subprocess when one request carries too many inputs (observed: ~600 inputs returning HTTP 400 `tokenize: EOF`), regardless of total token volume. `migrateVectorIndex` ([`src/functions/migrate-vector-index.ts`](src/functions/migrate-vector-index.ts)) hands the full set of session observations and memories to `embedBatch` on bulk reindex, so the largest sessions failed against such runners. `OpenAIEmbeddingProvider.embedBatch` ([`src/providers/embedding/openai.ts`](src/providers/embedding/openai.ts)) now splits into sequential sub-batches of this size (sequential to respect single-slot runners); the new `embedChunk` performs the single POST.
...
- **Session checkpoint is now trailing-edge** ([`src/functions/session-checkpoint.ts`](src/functions/session-checkpoint.ts)). The reactive `mem::session::checkpoint` (the `POST /agentmemory/session/checkpoint` path) fires only after a session has had no new activity for `AGENTMEMORY_IDLE_CHECKPOINT_MS` (measured from `updatedAt`, reset on any new observation), replacing the old leading-edge debounce that always fired a session's first idle and re-fired per idle thereafter. `AGENTMEMORY_CHECKPOINT_DEBOUNCE_MS` is now honored as a back-compat alias for the idle threshold (`0` disables the gate, restoring eager firing). The 6h `mem::session-sweep` finalize path decouples mark-done (age-based) from consolidation (activity-based), so an idle-checkpointed-then-abandoned session still completes at 6h with no redundant LLM call. Cuts consolidation load on weak single-slot local LLMs.
...
- **Background session processing repeated expensive work.** Nightly sweeps now load raw payloads once, release per-session observation locks during compression and consolidation, and defer finalization when new activity arrives. Idle checkpoints run only when graph extraction is enabled, reuse their pending-compression drain, and stop before the raw-payload scan while throttled. Semantic consolidation skips unchanged summary inputs unless forced.
- **OpenCode lifecycle capture discarded assistant text and ran corpus-wide maintenance when a session closed.** The plugin now captures terminal assistant text, keeps root-session ownership when child sessions start, resets compaction context before asynchronous work, and reopens explicitly resumed sessions. Session deletion ends only that session. Removed hooks no longer send observations the server intentionally drops, and diff capture sends file counts instead of full before-and-after contents.
- **Claude Code and Codex `Stop` finalized a session after every turn.** Both integrations now use one turn-scoped hook that records the final assistant message without ending or summarizing the session. `SessionStart` explicitly reopens resumed sessions, Codex adds `SubagentStart` and `SubagentStop`, and `connect codex --with-hooks` installs hooks even when MCP wiring already exists.
- **OpenCode plugin broke every single-file install with `ERR_MODULE_NOT_FOUND`** (regression from the 0.9.27 export-surface fix). To restore the plugin's runtime export surface to exactly `AgentmemoryCapturePlugin`, 0.9.27 moved the git commit-link helpers into a sibling `plugin/opencode/git-commit-metadata.ts` that the plugin imported via `import { ... } from "./git-commit-metadata"`. But the plugin is distributed as a SINGLE standalone file. `agentmemory connect opencode --with-plugin` ([`src/cli/connect/opencode.ts`](src/cli/connect/opencode.ts)), the manual `cp` docs, and nix all copy `agentmemory-capture.ts` alone into `~/.config/opencode/plugins/`, so the sibling was never present at runtime and the relative import threw `ERR_MODULE_NOT_FOUND` on load. Copying the sibling alongside it is not an option: OpenCode auto-loads every `.ts` in the plugins dir and its legacy loader invokes every runtime export as a plugin factory, so the sibling's 3 exports would recreate the original crash. Fixed by inlining `GitCommitMetadata` / `GIT_TIMEOUT_MS` / `runGit` / `tryGit` / `collectGitCommitMetadata` back into [`plugin/opencode/agentmemory-capture.ts`](plugin/opencode/agentmemory-capture.ts) as module-private (non-exported) declarations and deleting the sibling; the plugin is a single self-contained file again with exactly one runtime export. New `test/opencode-plugin-standalone.test.ts` copies the plugin alone into a fresh tmp dir and imports it in isolation to guard the regression.
- **OpenCode commit-link posted repo URLs with embedded credentials.** The `repo` field came straight from `git remote get-url origin`, which can carry HTTPS userinfo, SCP-style usernames or tokens, and query or fragment credentials. A module-private `sanitizeRepoUrl()` removes those fields before metadata leaves the machine and again before a queued retry is posted.
- **OpenCode commit-link permanently lost a link on a transient POST failure.** `linkCommitIfHeadChanged` advanced `lastSeenHeads` unconditionally after posting `/session/commit`, so a network error or 5xx dropped the commit link forever. The POST now reports success via a new `postOk()` helper and the head cursor only advances when the POST lands (2xx), so the next tool completion retries the same HEAD; a failed git metadata collection also leaves the cursor unadvanced to retry.
- **Image-backed deletion could release shared files before their owners were removed.** Memory and observation deletion now re-reads under keyed locks, removes every durable owner first, and stages ref release, session counts, and derived cleanup in a retryable journal. Governance, auto-forget, eviction, retention, and explicit forget use the same deletion path. Startup and session sweep drain interrupted releases.
- **Pending observation recovery ignored the compression opt-out and scanned all raw history.** Recovery now rebuilds synthetic observations when `AGENTMEMORY_AUTO_COMPRESS` is disabled. Per-session pending and raw-payload indexes give checkpoints, summaries, and session sweep targeted reads. Legacy raw payloads move through the explicit `raw-payloads-by-session` migration step instead of a corpus-wide scan inside live sweep.
- **Import search rebuilds could race live observation changes.** BM25 and vector indexes now rebuild off-side, stay queryable during the rebuild, and replay concurrent additions and removals before swapping.
- **Replace imports could report failure after publishing partial derived state.** Replace now records in-process KV mutations and conditionally restores keys after a thrown pre-commit failure when the key still contains the import's last successful write. If another writer changes a touched key between two import writes, rollback preserves the last value observed before the import overwrites it. Pending image-release recovery completes before rollback tracking begins, so a later import failure cannot resurrect a finished release after its file or counters changed. Rollback restores live and persisted search indexes, and physical image deletion waits until the import commit point. Post-commit image cleanup failures retain retry state without changing a committed import to failure. Isolated export, merge, and replace paths stay within the active agent's rows. Destructive replace is not atomic across process termination or the StateKV read-to-write gap because StateKV has no multi-key transaction or compare-and-set primitive.
- **Semantic consolidation watermarked malformed provider output.** The pipeline now requires one complete `<facts>` envelope, validates every fact and confidence before writing, and accepts a valid empty envelope without losing retryability after bad output.
- **OpenCode commit provenance attributed unrelated HEAD changes and dropped intermediate commits.** Each shell tool now gets a unique reflog marker. Checkout, reset, pull fast-forward, concurrent-session movement, and rebase bookkeeping are ignored. Ready overlapping tools persist independently to private per-commit outbox files, failed entries no longer block later commits, and queued links survive plugin reload or session deletion. Each outbox entry is bound to a fingerprint of its API destination and credential identity, so changing backends cannot send queued commit metadata to the wrong server.
- **Commit metadata could retain repository credentials or create ownerless isolated links.** OpenCode and standalone hooks strip HTTPS userinfo, SCP-style usernames or tokens, URL queries, and fragments before sending. REST and MCP commit reads sanitize legacy rows without rewriting storage, repository filters use the same canonical form, and isolated commit ingest holds the session lock from ownership validation through link creation. Isolated commit reads fail closed without an agent identity, and partially visible legacy links omit ambiguous repository and branch metadata. Config and backup files are written with mode `0600`; backup and temporary directories use mode `0700`.
- **Decimal graph chunk settings could drop extraction chunks.** `GRAPH_CHUNK_SIZE` and `GRAPH_CHUNK_CONCURRENCY` now accept only positive safe integers.
...
- **OpenCode plugin crashed the host on activation** (`"plugin config hook failed" ... null is not an object (evaluating 'N.config')`). Two compounding bugs, both in the commit-link capture feature: (1) the plugin factory changed from `async () => {...}` to `async (pluginInput) => {...}` to read `pluginInput.directory`, but accessed it unguarded; OpenCode's own crash reporting made it look like a `Provider.list` failure. Fixed with `pluginInput?.directory || process.cwd()` fallback. (2) After that fix the same crash recurred: OpenCode's legacy plugin loader iterates every named export of a plugin file as a candidate entrypoint, and the newly-exported `collectGitCommitMetadata` helper (exported only for its own test file) got invoked as if it were a plugin, returned `null`, and crashed on `.config`. Fixed by moving `collectGitCommitMetadata`/`GitCommitMetadata`/`runGit`/`tryGit` out of `plugin/opencode/agentmemory-capture.ts` into a new `plugin/opencode/git-commit-metadata.ts`, restoring the plugin entrypoint's export surface to exactly `AgentmemoryCapturePlugin`.
