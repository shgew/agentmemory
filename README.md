...
| **OpenCode (full plugin)** | `plugin/opencode/` | 22 auto-capture hooks covering session lifecycle, messages, tools, errors. Skills tree under `plugin/skills/` (16 native skills, 9 invocable; OpenCode promotes them into its slash-command palette automatically). Copy `plugin/opencode/` into your OpenCode workspace and add the plugin entry to `opencode.json`. See [`plugin/opencode/README.md`](plugin/opencode/README.md) for the full hook table + gap analysis. |
...
| Skill | `/health` | Probe server, surface diagnostics |
